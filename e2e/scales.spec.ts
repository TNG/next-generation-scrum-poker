...
test('supports creating and using custom scales', async ({ page, context }) => {
  ...
  // Both users should see custom scale
  await cardPage.assertCardsAre(['XS', 'S', 'M', 'L', 'observer']);
  await secondCardPage.assertCardsAre(['XS', 'S', 'M', 'L', 'observer']);
  ...
  // Verify custom values appear in results
